...
local VapeWindow = {}
VapeWindow.__index = VapeWindow

function Library:CreateVapeWindow(Data)
    local win = self:Window(Data)
    return setmetatable({_win = win, _subpages = {}, _sections = {}}, VapeWindow)
end

function VapeWindow:CreateCategory(name)
    local page = self._win:Page({Icon = "109391165290124"})
    local subpage = page:SubPage({Name = name})
    ...
    cat._leftSec = subpage:Section({Name = name, Side = "Left"})
    cat._rightSec = subpage:Section({Name = name, Side = "Right"})

    local modMeta = {}
    modMeta.__index = modMeta

    function cat:CreateModule(Data)
        local mod = {
            Name = Data.Name,
            Enabled = false,
            _connections = {},
        ...
        setmetatable(mod, modMeta)

        local toggle = cat._leftSec:Toggle({
            Name = Data.Name,
            Flag = Data.Name,
        ...
        function mod:Toggle()
            self._toggle:Set(not self.Enabled)
        end

        function mod:Clean(connection)
            if type(connection) == "function" then
                table.insert(self._cleanups, connection)
            elseif connection and connection.Connection then
                table.insert(self._connections, connection.Connection)
        ...
        function mod:CreateSlider(d)
            return cat:_createElement("Slider", d, self)
        end

        function mod:CreateToggle(d)
            return cat:_createElement("Toggle", d, self)
        end

        function mod:CreateTargets(d)
            return cat:_createTargets(d, self)
        end

        function mod:CreateColorSlider(d)
            return cat:_createElement("ColorSlider", d, self)
        end

        function mod:CreateTwoSlider(d)
            return cat:_createElement("TwoSlider", d, self)
        end

        cat.Modules[Data.Name] = mod
        cat.Options[Data.Name] = mod
        return mod
    end

    function cat:_createElement(elemType, Data, parent)
        local sec = cat._leftSec
        local elem = {
            Value = Data.Default,
            Enabled = Data.Default == true,
            Object = nil,
            Hue = 0, Sat = 1, Value2 = 1,
        }
        ...
        if elemType == "Dropdown" then
            ...
            function elem:SetVisibility(bool)
                drop:SetVisibility(bool)
            end

        elseif elemType == "Slider" then
            local suffix = type(Data.Suffix) == "function" and "" or (Data.Suffix or "")
            local slide = sec:Slider({
                Name = Data.Name,
                Flag = Data.Name..(parent and parent.Name or ""),
                Min = Data.Min or 0,
                Max = Data.Max or 100,
                Default = Data.Default or Data.Min or 0,
                Decimals = Data.Decimal or Data.Decimals or 1,
                Suffix = suffix,
                Callback = function(val)
                    elem.Value = val
                    if type(Data.Suffix) == "function" then
                    end
                    if Data.Function then Data.Function(val) end
                    if Data.Callback then Data.Callback(val) end
                end
            ...
            local mt = getmetatable(elem) or {}
            mt.__index = function(t, k)
                if k == "Value" then return slide:Get() end
                return rawget(t, k)
            end
            setmetatable(elem, mt)

            function elem:GetRandomValue()
                return self.Value
            end

            function elem:SetVisibility(bool)
                slide:SetVisibility(bool)
            end
        ...
        elseif elemType == "Toggle" then
            ...
            function elem:SetVisibility(bool)
                tog:SetVisiblity(bool)
            end

        elseif elemType == "ColorSlider" then
            local label = sec:Label(Data.Name)
            local cp = label:Colorpicker({
                ...
                Callback = function(color, alpha)
                    ...
                    elem.Sat = s
                    elem.Value2 = v
                    if Data.Function then Data.Function(h, s, v) end
                end
            })
            elem.Object = label
            elem._cp = cp
        ...
        elseif elemType == "TwoSlider" then
            local slideMin = sec:Slider({
                Name = Data.Name .. " Min",
                Flag = Data.Name.."Min"..(parent and parent.Name or ""),
                Min = Data.Min or 0,
                Max = Data.Max or 100,
                Default = Data.DefaultMin or Data.Min or 0,
                Callback = function(val)
                    elem.MinValue = val
                end
            })
            local slideMax = sec:Slider({
                Name = Data.Name .. " Max",
                Flag = Data.Name.."Max"..(parent and parent.Name or ""),
                Min = Data.Min or 0,
                Max = Data.Max or 100,
                Default = Data.DefaultMax or Data.Max or 100,
                Callback = function(val)
                    elem.MaxValue = val
                end
            })
            elem.MinValue = Data.DefaultMin or Data.Min or 0
            elem.MaxValue = Data.DefaultMax or Data.Max or 100
            elem.Object = slideMin

            function elem:GetRandomValue()
                return math.random(
                    math.floor(self.MinValue),
                    math.floor(self.MaxValue)
                ) + math.random() * (self.MaxValue - math.floor(self.MaxValue))
            end
        end
    ...
    function cat:_createTargets(Data, parent)
        local targets = {
            Players = {Enabled = Data and Data.Players or false},
            NPCs    = {Enabled = Data and Data.NPCs or false},
            Walls   = {Enabled = Data and Data.Walls or false},
        }
        ...
        local pt = sec:Toggle({
            Name = "Target Players",
            Flag = "TargetPlayers"..(parent and parent.Name or ""),
            Default = targets.Players.Enabled,
            Callback = function(val) targets.Players.Enabled = val end
        })
        local nt = sec:Toggle({
            Name = "Target NPCs",
            Flag = "TargetNPCs"..(parent and parent.Name or ""),
            Default = targets.NPCs.Enabled,
            Callback = function(val) targets.NPCs.Enabled = val end
        })
        local wt = sec:Toggle({
            Name = "Wall Check",
            Flag = "WallCheck"..(parent and parent.Name or ""),
            Default = targets.Walls.Enabled,
            Callback = function(val) targets.Walls.Enabled = val end
        })

        targets.Players.Object = pt
        targets.NPCs.Object = nt
        targets.Walls.Object = wt
